...
//  IOS-EQ04
//
//  Created by Ryzlane Arsac-Gothière & Quentin on 27/11/2018.
...
class PartyViewController: UIViewController, UITextFieldDelegate , UIScrollViewDelegate, UICollectionViewDataSource, UICollectionViewDelegate, UITableViewDelegate, UITableViewDataSource, UITabBarDelegate{
    ...
    @IBAction func firstAidAction(_ sender: Any) {
        let myStoryboard = UIStoryboard(name: "Main", bundle: nil)
        let EmergencyViewController = myStoryboard.instantiateViewController(withIdentifier: "EmergencyViewController")
        self.present(EmergencyViewController, animated: false, completion: nil)
    }
    @IBAction func CreditCardAction(_ sender: Any) {
        let myStoryboard = UIStoryboard(name: "Main", bundle: nil)
        let CreditCardViewController = myStoryboard.instantiateViewController(withIdentifier: "CreditCardViewController")
        self.present(CreditCardViewController, animated: false, completion: nil)
    }
    ...
    let jaugechildrenView: UIView = {
        
        let jauge = UIView()
        jauge.frame = CGRect(x:0, y: 0, width:200, height: 50)
        jauge.backgroundColor = UIColor(hex: "F89934")
    ...
    override func viewDidLoad() {
        ...
        jaugeView.addSubview(jaugechildrenView)
...
//            print("fle")
    ...
    func initJauge() {
                        ...
                        db.collection("users").document(document.documentID).collection("drinks").getDocuments(completion: { (query, err) in
                            if let err = err {
                                ...
                                for doc in query!.documents {
                                    let drink = RegisteredDrink(dictionary: doc.data())
                                    
                                    let formatter = DateFormatter()
                                    formatter.timeZone = TimeZone.current
                                    if let date = drink?.date {
                                        formatter.dateFormat = "yyyy-MM-dd HH:mm"
                                        let dateN = formatter.date(from: date)
                                        if let newdate = dateN {
                                            var istoday = Calendar.current.isDateInToday(newdate)
                                            if(istoday) {
                                                count += 1
                                            }
                                        }
        ...
        jaugechildrenView.bottomAnchor.constraint(equalTo: jaugechildrenView.topAnchor, constant: 0).isActive = true
